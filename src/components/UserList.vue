<template>
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      <div>
        <h2 class="text-xl font-semibold text-slate-800 dark:text-slate-100">Quản lý người dùng</h2>
    ...
    <div v-else-if="users.length === 0" class="text-center py-12 text-slate-500 dark:text-slate-400">
        ...
        <table class="w-full">
          <thead>
            <tr class="bg-slate-50 dark:bg-slate-700">
              <th class="px-6 py-3 text-left text-sm font-semibold text-slate-600 dark:text-slate-200">Người dùng</th>
              <th class="px-6 py-3 text-left text-sm font-semibold text-slate-600 dark:text-slate-200">Email</th>
              <th class="px-6 py-3 text-center text-sm font-semibold text-slate-600 dark:text-slate-200">Vai trò</th>
              <th class="px-6 py-3 text-center text-sm font-semibold text-slate-600 dark:text-slate-200">Trạng thái</th>
              <th class="px-6 py-3 text-center text-sm font-semibold text-slate-600 dark:text-slate-200">Thao tác</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-slate-200 dark:divide-slate-600">
            <tr v-for="user in filteredUsers" :key="user.id" class="hover:bg-slate-50 dark:hover:bg-slate-700/50">
              <td class="px-6 py-4">
                <div class="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p class="text-sm text-slate-500 dark:text-slate-400">Tham gia: {{ formatDate(user.createdAt) }}</p>
                  </div>
                </div>
              </td>
    ...
    <div v-if="showRoleModal" class="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div class="relative bg-white dark:bg-slate-800 rounded-xl shadow-lg w-full max-w-md p-6">
        <h3 class="text-lg font-semibold text-slate-800 dark:text-slate-100 mb-4">Phân quyền người dùng</h3>
        ...
        <div class="flex justify-end gap-3 mt-6">
          ...
          >
            <i v-if="isUpdating" class="fas fa-spinner fa-spin mr-2" />
            Cập nhật vai trò
          </button>
        </div>
      </div>
    ...
    <div v-if="showLockModal" class="fixed inset-0 z-50 flex items-center justify-center">
      <div class="absolute inset-0 bg-black/50 backdrop-blur-sm" @click="showLockModal = false" />
      <div class="relative bg-white dark:bg-slate-800 rounded-xl shadow-lg w-full max-w-md p-6">
        <h3 class="text-lg font-semibold text-slate-800 dark:text-slate-100 mb-4">Khóa tài khoản người dùng</h3>

        <div class="mb-4">
          <p class="text-slate-600 dark:text-slate-400">
            Bạn đang khóa tài khoản của người dùng
            <span class="font-medium text-slate-800 dark:text-slate-200">{{ selectedUser?.name }}</span>
          </p>
        </div>

        <form class="space-y-4" @submit.prevent="lockUser">
          <div>
    ...
    <div v-if="showUnlockModal" class="fixed inset-0 z-50 flex items-center justify-center">
      <div class="absolute inset-0 bg-black/50 backdrop-blur-sm" @click="showUnlockModal = false" />
      <div class="relative bg-white dark:bg-slate-800 rounded-xl shadow-lg w-full max-w-md p-6">
        <h3 class="text-lg font-semibold text-slate-800 dark:text-slate-100 mb-4">Mở khóa tài khoản</h3>
        ...
        <div class="space-y-4">
          ...
          <p class="text-slate-600 dark:text-slate-400">Bạn có chắc chắn muốn mở khóa tài khoản này không?</p>
        </div>
...
const filteredUsers = computed(() => {
  ...
  if (statusFilter.value !== 'all') {
    result = result.filter((user) => (statusFilter.value === 'locked' ? user.isLocked : !user.isLocked));
  }
...
const fetchUsers = async () => {
  ...
  try {
    ...
    const response = await proxy.$http.get(Endpoint.getUsers);
    if (response.succeeded) {
      users.value = response.result;
    } else {
      error.value = response.errors ? response.errors.join(', ') : 'Không thể tải danh sách người dùng';
      console.error('Fetch users error:', response);
    }
  } catch (err) {
...
const updateUserRole = async () => {
  ...
  try {
    ...
    const response = await proxy.$http.put(Endpoint.updateUserRole(selectedUser.value.id), {
      role: newRole.value
    });
    ...
    if (response.succeeded) {
      ...
      console.error('Error updating user role:', response.errors ? response.errors.join(', ') : response);
    }
  } catch (err) {
...
const lockUser = async () => {
  ...
  try {
    ...
    const response = await proxy.$http.post(Endpoint.lockUser(selectedUser.value.id), {
    ...
    if (response.succeeded) {
      ...
      console.error('Error locking user:', response.errors ? response.errors.join(', ') : response);
    }
  } catch (err) {
...
const unlockUser = async () => {
  ...
  try {
    ...
    const response = await proxy.$http.post(Endpoint.unlockUser(selectedUser.value.id));
    ...
    if (response.succeeded) {
      ...
      console.error('Error unlocking user:', response.errors ? response.errors.join(', ') : response);
    }
  } catch (err) {
