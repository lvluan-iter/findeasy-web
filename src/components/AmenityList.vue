<template>
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      <div>
        <h2 class="text-xl font-semibold text-slate-800 dark:text-slate-100">Quản lý tiện ích</h2>
        <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Quản lý các tiện ích cơ bản của bất động sản</p>
      </div>
      <button
    ...
    <div v-else-if="amenities.length === 0" class="text-center py-12 text-slate-500 dark:text-slate-400">
    ...
    <div v-else class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
      <div
        ...
        :key="amenity.id"
        class="bg-white dark:bg-slate-800 rounded-lg shadow-sm hover:shadow-md transition-shadow p-4 flex flex-col items-center group relative"
      >
        <div
          class="w-12 h-12 flex items-center justify-center text-2xl mb-3 bg-blue-50 dark:bg-blue-500/10 rounded-lg text-blue-600 dark:text-blue-400"
        >
    ...
    <div v-if="isModalOpen" class="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div class="relative bg-white dark:bg-slate-800 rounded-xl shadow-lg w-full max-w-md p-6">
        ...
        <form class="space-y-4" @submit.prevent="saveAmenity">
          <div>
            <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1"> Tên tiện ích </label>
            <input
          ...
          <div>
            ...
            <div class="relative">
              <input
                ...
                required
                placeholder="fas fa-..."
                class="w-full px-4 py-2 pl-10 rounded-lg bg-slate-100 dark:bg-slate-700 border-0 focus:ring-2 focus:ring-[rgb(10,115,192)] dark:text-slate-100"
              />
              <i
          ...
          <div class="p-4 bg-slate-50 dark:bg-slate-700/50 rounded-lg">
            <p class="text-sm text-slate-500 dark:text-slate-400 mb-2">Preview:</p>
            <div class="flex items-center gap-4">
              <div
                class="w-10 h-10 flex items-center justify-center text-xl bg-white dark:bg-slate-700 rounded-lg text-blue-600 dark:text-blue-400"
              >
          ...
          <div>
            <p class="text-sm text-slate-500 dark:text-slate-400 mb-2">Icons phổ biến:</p>
...
const filteredAmenities = computed(() => {
  ...
  return amenities.value.filter(
    (amenity) => amenity.name.toLowerCase().includes(term) || amenity.icon.toLowerCase().includes(term)
  );
});
...
const fetchAmenities = async () => {
  ...
  try {
    ...
    if (!response || !response.succeeded) {
      const msg = response && response.errors ? response.errors.join(', ') : 'Không thể tải danh sách tiện ích';
      throw new Error(msg);
    }

    const res = response.result;
    // support array or paged result
    if (Array.isArray(res)) {
      amenities.value = res;
    } else if (res && Array.isArray(res.content)) {
      amenities.value = res.content;
    } else {
      amenities.value = res ? (Array.isArray(res) ? res : [res]) : [];
    }
  } catch (err) {
    error.value = err.message || 'Không thể tải danh sách tiện ích';
    console.error(err);
  } finally {
...
const saveAmenity = async () => {
  try {
    ...
    if (editingAmenity.value) {
      response = await proxy.$http.put(Endpoint.updateAmenity(editingAmenity.value.id), payload);
    } else {
      response = await proxy.$http.post(Endpoint.createAmenity, payload);
    }

    if (!response || !response.succeeded) {
      const msg = response && response.errors ? response.errors.join(', ') : 'Có lỗi xảy ra khi lưu tiện ích';
      formError.value = msg;
      console.error('Save amenity failed:', response);
      return;
    }
    ...
    formError.value = err.message || 'Không thể lưu tiện ích. Vui lòng thử lại.';
    console.error(err);
  } finally {
...
const deleteAmenity = async () => {
  ...
  try {
    ...
    if (!response || !response.succeeded) {
      const msg = response && response.errors ? response.errors.join(', ') : 'Không thể xóa tiện ích';
      throw new Error(msg);
    }
