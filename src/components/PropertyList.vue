<template>
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      <div>
        <h2 class="text-xl font-semibold text-slate-800 dark:text-slate-100">Quản lý tin đăng</h2>
        <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Duyệt và quản lý các tin đăng bất động sản</p>
      </div>
    </div>
    ...
    <div class="relative">
      ...
      <input
        ...
        class="w-full pl-10 pr-4 py-2 rounded-lg bg-white dark:bg-slate-800 border-0 focus:ring-2 focus:ring-[rgb(10,115,192)] dark:text-slate-100 shadow-sm"
      />
    </div>
    ...
    <div v-else-if="filteredListings.length === 0" class="text-center py-12 text-slate-500 dark:text-slate-400">
      ...
      <table class="w-full">
        <thead>
          <tr class="border-b border-slate-200 dark:border-slate-700">
            <th class="text-left py-3 px-4 text-sm font-semibold text-slate-600 dark:text-slate-300">Tiêu đề</th>
            <th class="text-left py-3 px-4 text-sm font-semibold text-slate-600 dark:text-slate-300">Người đăng</th>
            <th class="text-left py-3 px-4 text-sm font-semibold text-slate-600 dark:text-slate-300">Ngày đăng</th>
            <th class="text-left py-3 px-4 text-sm font-semibold text-slate-600 dark:text-slate-300">Trạng thái</th>
            <th class="text-right py-3 px-4 text-sm font-semibold text-slate-600 dark:text-slate-300">Thao tác</th>
          </tr>
        </thead>
        <tbody class="divide-y divide-slate-200 dark:divide-slate-700">
          ...
          >
            ...
            <td class="py-3 px-4">
              <span class="px-2 py-1 text-xs rounded-full" :class="getStatusClass(listing)">
            ...
            <td class="py-3 px-4">
              <div class="flex justify-end gap-2">
                <button
                  class="p-1.5 text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg transition-colors"
                ...
                <template v-if="!listing.approved && !listing.locked">
                  <button
                    class="p-1.5 text-green-600 hover:bg-green-50 dark:hover:bg-green-500/10 rounded-lg transition-colors"
    ...
    <div v-if="showLockModal" class="fixed inset-0 z-50 flex items-center justify-center">
      <div class="absolute inset-0 bg-black/50 backdrop-blur-sm" @click="closeLockModal" />
      <div class="relative bg-white dark:bg-slate-800 rounded-xl shadow-lg w-full max-w-md p-6">
        <h3 class="text-lg font-semibold text-slate-800 dark:text-slate-100 mb-4">Khóa tin đăng</h3>
        ...
        <form @submit.prevent="lockListing">
          ...
          <div class="flex justify-end gap-3 mt-6">
            ...
            <button
              ...
              :disabled="isRunning"
            >
              <i v-if="isRunning" class="fas fa-spinner fa-spin mr-2" />
              Khóa
            </button>
          </div>
        </form>
    ...
    <div v-if="showRejectModal" class="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div class="relative bg-white dark:bg-slate-800 rounded-xl shadow-lg w-full max-w-md p-6">
        <h3 class="text-lg font-semibold text-slate-800 dark:text-slate-100 mb-4">Từ chối tin đăng</h3>

        <form @submit.prevent="rejectListing">
          <div>
            <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1"> Lý do từ chối </label>
            <textarea
    ...
    <div v-if="showPreviewModal" class="fixed inset-0 z-50 flex items-center justify-center">
      <div class="absolute inset-0 bg-black/50 backdrop-blur-sm" @click="closePreviewModal" />
      <div
        class="relative bg-white dark:bg-slate-800 rounded-xl shadow-lg w-full max-w-4xl p-6 max-h-[90vh] overflow-y-auto"
      >
        <div class="flex justify-between items-start mb-4">
          <h3 class="text-lg font-semibold text-slate-800 dark:text-slate-100">Chi tiết tin đăng</h3>
          <button
        ...
        <div v-if="selectedListing" class="space-y-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <h4 class="font-semibold text-slate-800 dark:text-slate-100 mb-2">Thông tin cơ bản</h4>
              <dl class="space-y-2">
                <div class="flex justify-between">
                  <dt class="text-slate-500 dark:text-slate-400">Giá</dt>
                  <dd class="font-medium text-slate-800 dark:text-slate-200">
                    {{ formatPrice(selectedListing.price) }}
                  </dd>
                </div>
                <div class="flex justify-between">
                  <dt class="text-slate-500 dark:text-slate-400">Diện tích</dt>
                  <dd class="font-medium text-slate-800 dark:text-slate-200">{{ selectedListing.area }} m²</dd>
                </div>
                <div class="flex justify-between">
                  <dt class="text-slate-500 dark:text-slate-400">Loại BĐS</dt>
                  <dd class="font-medium text-slate-800 dark:text-slate-200">
            ...
            <div>
              <h4 class="font-semibold text-slate-800 dark:text-slate-100 mb-2">Địa chỉ</h4>
              <p class="text-slate-600 dark:text-slate-300">
          ...
          <div>
            <h4 class="font-semibold text-slate-800 dark:text-slate-100 mb-2">Mô tả</h4>
          ...
          <div>
            <h4 class="font-semibold text-slate-800 dark:text-slate-100 mb-2">Tiện ích</h4>
...
const fetchListings = async () => {
  ...
  try {
    ...
    if (response.succeeded) {
      listings.value = response.result;
    } else {
      throw new Error(response.errors ? response.errors.join(', ') : 'Failed to fetch listings');
    }
  } catch (err) {
...
const approveListing = async (listing) => {
  try {
    ...
    if (!response.succeeded) {
      throw new Error(response.errors ? response.errors.join(', ') : 'Failed to approve listing');
    }
...
const rejectListing = async () => {
  ...
  try {
    ...
    if (!response.succeeded) {
      throw new Error(response.errors ? response.errors.join(', ') : 'Failed to reject listing');
    }
...
const lockListing = async () => {
  ...
  try {
    ...
    if (!response.succeeded) {
      throw new Error(response.errors ? response.errors.join(', ') : 'Failed to lock listing');
    }
...
const unlockListing = async (listing) => {
  try {
    ...
    if (!response.succeeded) {
      throw new Error(response.errors ? response.errors.join(', ') : 'Failed to unlock listing');
    }
