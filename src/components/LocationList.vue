    ...
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">
          Quản lý các địa điểm bất động sản trong hệ thống
        </p>
      </div>
      <button
...
const saveLocation = async () => {
  try {
    ...
    const url = editingLocation.value 
      ? `${API_URL}/${editingLocation.value.id}`
      : API_URL
    ...
    if (!response.ok) {
      throw new Error('Có lỗi xảy ra khi lưu địa điểm')
    }
...
const deleteLocation = async () => {
  ...
  try {
    const response = await fetch(`${API_URL}/${locationToDelete.value.id}`, {
      method: 'DELETE',
      headers: {
        'Authorization': `Bearer ${localStorage.getItem('jwt')}`
      }
