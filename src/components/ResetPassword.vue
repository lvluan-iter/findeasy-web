  ...
  <div>
    ...
    <div
      ...
      <span class="text-gray-600">Fill with your mail to receive instructions on how to reset your
        password.</span>
      <form class="mt-5" @submit.prevent="handleResetPasswordSubmit">
        <div class="form-group relative mb-5 text-left animate-fade-in-up">
          <label :class="{ 'active': newPassword.length > 0 || isFocusedNewPassword }" for="new-password"
            class="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-500 px-1.5 transition-all duration-300 pointer-events-none text-base">New
            Password</label>
          <input v-model="newPassword"
            ...
            type="password" required @focus="isFocusedNewPassword = true" @blur="isFocusedNewPassword = false">
        </div>
        <button
...
const handleResetPasswordSubmit = async () => {
  try {
    const response = await proxy.$http.post(Endpoint.resetPassword, {
